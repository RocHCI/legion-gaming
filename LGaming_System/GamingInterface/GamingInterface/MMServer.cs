...
namespace GamingInterface
{
    ...
    public class MMServer
    {
        ...
        public int leader = -1;
        public double leaderTime = 0;
        public int myport = -1;

        String logFilePath = "leader.log";
        ...
        private void ProcessReceive(SocketAsyncEventArgs e)
        {
            ...
            if (e.BytesTransferred > 0 && e.SocketError == SocketError.Success)
            {
                ...
                string recievedString = System.Text.ASCIIEncoding.ASCII.GetString(e.Buffer);

                string[] recievedSubstrings = recievedString.Split(';');
                int endOfString = recievedSubstrings[2].IndexOf('!');
                recievedSubstrings[2] = recievedSubstrings[2].Substring(0, endOfString);

                if (String.Compare(recievedSubstrings[0], "NOLEADER") != 0)
                {
                    if (leader != int.Parse(recievedSubstrings[0]))
                    {
                        using (StreamWriter logWriter = new StreamWriter(logFilePath, true))
                        {
                            logWriter.WriteLine(DateTime.Now + "    " + int.Parse(recievedSubstrings[0]));
                        }
                    }
                    leader = int.Parse(recievedSubstrings[0]);
                }
                else
                {
                    leader = -1;
                ...
                leaderTime = double.Parse(recievedSubstrings[1]);
