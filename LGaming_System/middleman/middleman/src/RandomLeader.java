...
public class RandomLeader extends InputMediator {
	LinkedList<Integer> players;
	int leader = -1;
	long leaderMaxTimeSpan = 10000;
	long leaderStartTime = 0;

	
	RandomLeader(Socket server) {
		super(server);
	...
	public byte[] input(byte[] b) {
		...
		if( elapsedTime > leaderMaxTimeSpan ) {
			...
			System.out.println("Old leader: " + leader + " --> new candidates: " + players.size());
