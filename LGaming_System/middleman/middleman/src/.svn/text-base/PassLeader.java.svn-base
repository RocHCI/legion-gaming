...
// TODO: Currently not running
public class PassLeader extends InputMediator {
	...
	long leaderMinTimeSpan = 10000;
	
	PassLeader(Socket server) {
		super(server);
	...
	public byte[] input(byte[] b) {
		...
		if( !players.contains(uID) ) {
			...
			if( curStr.substring(curStr.length()-1, curStr.length()).equals("1") ) {
				...
				System.out.println("LEADER'S SELECT PRESSED!");
				...
				System.out.println("Old leader: " + oldLeader + " --> new candidates: " + players.size());
				while( leader == oldLeader ) {
					leader = players.get(rand.nextInt(players.size()));
				}
