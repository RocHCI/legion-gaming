...
abstract public class InputMediator {
	...
	LinkedList<Integer> selPressedPrev = new LinkedList<Integer>();
	
	// Max amount of noise we expect for analog input
	int analogNoise = 10;
	
	// This has to be global to "hold down" buttons while waiting for a certain player's input
	ControllerInput sharedInput = new ControllerInput(128,128,128,128,"0000000000000000");
	
	ControllerInput zeroInput = new ControllerInput(128,128,128,128,"0000000000000000");
	int[] attribution = {-1,-1,-1,-1, -1,-1,-1,-1};
	final String[] segmentNames = {"joyLx", "joyLy", "joyRx", "joyRy", "triggers", "dpad", "buttons", "special"};
	...
	public InputMediator(Socket server) {
		...
		activePlayers = new LinkedList<Integer>();
	...
	// If select is pressed, toggle pause mode
	public void handlePM(int uID, String curInput) {
		...
		if( curStr.substring(curStr.length()-1, curStr.length()).equals("1") ) {
			if( activePlayers.contains(uID) && !selPressedPrev.contains((Integer)uID) ) {
				activePlayers.remove((Integer)uID);
				System.out.println("-=-=-=-=-= activePlayers " + uID + ": false");
			} else if( !activePlayers.contains(uID) && !selPressedPrev.contains((Integer)uID) ){
				activePlayers.add((Integer)uID);
				System.out.println("-=-=-=-=-= activePlayers " + uID + ": true");
			}
		...
		System.out.println("-=-=-= new activePlayers: " + activePlayers.toString());
	}
	...
	public ControllerInput parseStringToCI(String buttonInput) {
		// Split up the input
		String[] tmpjoy = buttonInput.split(" ");
		//System.out.println("----- origbuttonInput: " + buttonInput);
		buttonInput = buttonInput.substring(0,buttonInput.lastIndexOf(' '));
		buttonInput = buttonInput.substring(0,buttonInput.lastIndexOf(' '));
		buttonInput = buttonInput.substring(buttonInput.lastIndexOf(' ')+1);
		//System.out.println("----- buttonInput: " + buttonInput);
		
		// --- UGLY: Change the grouping to match the desired control groups ---
		//System.out.println("++++++++++++++ Before inputBuffer (buttonInput): " + buttonInput);
		StringBuffer inputBuffer = new StringBuffer(buttonInput); 
		char R3 = inputBuffer.charAt(5);
		char L3 = inputBuffer.charAt(2);
		inputBuffer.deleteCharAt(5);
		inputBuffer.deleteCharAt(2);
		//System.out.println("++++++++++++++ Middle inputBuffer (inputBuffer): " + inputBuffer);
		inputBuffer.append(L3);
		inputBuffer.append(R3);
		//System.out.println("++++++++++++++ After inputBuffer (inputBuffer): " + inputBuffer);
		//System.out.println("++++++++++++++ L3: " + L3);
		//System.out.println("++++++++++++++ R3: " + R3);
		buttonInput = inputBuffer.toString();
		// ---- END UGLY ----
		return new ControllerInput(Integer.parseInt(tmpjoy[0]), Integer.parseInt(tmpjoy[1]), Integer.parseInt(tmpjoy[2]), Integer.parseInt(tmpjoy[3]), buttonInput);
	}
